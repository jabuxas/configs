...
main =
  do
    ...
    $ debugManageHookOn "M-S-d"
      ...
      . myEwmhFullscreen -- breaks league of legends
...
-- Default Launcher
myLauncher :: String
myLauncher = myHomeDir ++ "/.config/rofi/launchers/type-2/launcher.sh"
...
-- Workspaces
myWorkspaces :: [String]
myWorkspaces = ["one", "two", "three", "four", "five", "six", "seven", "eight", "nine", "ten", "eleven"]
...
fullscreenEventHookNoLeagueClient ev@(ClientMessageEvent {ev_event_display = dpy, ev_window = win}) =
  liftIO (resClass <$> getClassHint dpy win) >>= \case
    "leagueclientux.exe" -> return $ All True
    _ -> fullscreenEventHook ev
  ...
  where
    ...
    applications =
      [ ("M-<Return>", spawn myTerminal),
        ...
        ("M-b", spawn myBrowser),
        ("M-S-s", spawn "steam --noverifyfiles -cef-enable-debugging"),
        ("M-C-s", spawn "~/sfp.sh"),
